...
variable "enable_indentity_service_config" {
  type        = bool
...
variable "cluster_name" {
  type        = string
  description = "The name of the cluster on which the GKE backup will be enabled"
}

variable "project_id" {
  type        = string
  description = "The project id for which GKE backup will be enabled"
}

variable "region" {
  type        = string
  description = "The region where the GKE backup will be enabled."
  default     = "europe-west1"
}

variable "cluster_path" {
  type        = string
  description = "The relative path to the cluster. For regional 'projects/PROJECT_ID/locations/REGION/clusters/CLUSTER_NAME', for zonal 'projects/PROJECT_ID/locations/ZONE/clusters/CLUSTER_NAME'."
}

variable "encryption_key" {
  type        = string
  description = "Defines the path to a customer-managed encryption key."
}

variable "cron_schedule" {
  type        = string
  description = "Defines a schedule that automatically creates backups and supports standard cron syntax."
  default     = "0 21 * * *"
}
variable "retain_days" {
  type        = number
  description = "The number of days to keep the backup. Backups for this plan are automatically deleted after this period expires."
  default     = 30
}
variable "delete_lock_days" {
  type        = number
  description = "The number of days during which backups cannot be deleted. If this value is set, then backups created for that plan cannot be deleted (either automatically or manually) until that lifetime expires."
  default     = 30
}
*/
###### Artifact Registry #####
...
variable "apply_roles" {
  type        = bool
  description = "SEt to true if IAM roles can be applied on repositories."
  default     = false
}
*/
##### Binary Authorization #####

/*variable "binary_auth_description" {
  type        = string
  description = "A descriptive comment."
}

variable "binary_auth_region" {
  type        = string
